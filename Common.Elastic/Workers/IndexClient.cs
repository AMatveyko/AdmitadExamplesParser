...
namespace Common.Elastic.Workers
{
    public sealed class IndexClient : IIndexClient
    {
        ...
        public static IIndexClient Create( ElasticSearchClientSettings settings, BackgroundBaseContext context ) =>
            new IndexClient( settings, context );

        public List<ProductPart> SearchProductsByOffersIds( string[] ids )
        {
            const string time = "1m";

            var preparedIds = ids.Select( i => i.ToLower() );
            
            var searchResponse = _client.Search<ProductPart>(s => 
                s.Size( 1000 ).Query(q => q.Bool( b => b.Must( t => t.Terms( terms => terms.Field( p => p.OfferIds ).Terms( preparedIds ) ) ) ) )
                .Scroll( time )
            );
        ...
        public void UpdateProductsAfterDeletingOffers< T >( IReadOnlyList<T> products ) where T : ProductPart
        {
            var result = _client.Bulk( b => b.UpdateMany<ProductPart>( products, ( descriptor, part ) => descriptor.Doc( part ) ) );
            CheckResponse( result );
        }

        public List<ProductPart> GetProductsByIds( string[] ids )
        {
        ...
        public void UpsertProducts<T>( IReadOnlyList<T> products ) where T : class, IIndexedEntities
        {
            var position = 0;
